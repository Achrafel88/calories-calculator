...
export const Dashboard = () => {
  ...
  return (
    <div className="pb-24 pt-6 px-4 max-w-md mx-auto space-y-6 min-h-screen">
      ...
      {/* Water Tracker */}
      <div className="bg-[#16a34a] rounded-[2.5rem] p-6 text-white flex justify-between items-center shadow-lg">
      ...
      <AnimatePresence>
        {isAddingFood && (
          <div className="fixed inset-0 z-50 flex items-end justify-center p-4">
             ...
             <motion.div initial={{ y: "100%" }} animate={{ y: 0 }} exit={{ y: "100%" }} className="relative bg-white w-full max-w-md rounded-[2.5rem] p-8 shadow-2xl">
               <input autoFocus value={foodInput} onChange={(e) => handleInputChange(e.target.value)} className="w-full p-4 bg-green-50 rounded-2xl outline-none focus:ring-2 focus:ring-green-500" placeholder="Search food..." />
               <div className="max-h-40 overflow-y-auto mt-2">
                 {suggestions.map(s => <button key={s.id} onClick={() => selectSuggestion(s)} className="w-full text-left p-3 border-b border-gray-100 hover:bg-green-50">{s.name}</button>)}
               </div>
               <button onClick={handleAddFood} className="w-full mt-4 p-4 bg-[#16a34a] text-white rounded-2xl font-bold hover:bg-[#15803d] transition-all">Add Food</button>
             </motion.div>
          </div>
        )}
      </AnimatePresence>
